...
class TestCreateOrder:
    ...
    def test_create_order_with_token(self, user_create_and_delete):
        ...
        order = Order()
        helper = Helpers()
        response = order.create_order(helper.add_ingredients(), token)
    ...
    def test_create_order_with_incorrect_hash_error(self):
        token = None
        order = Order()
        response = order.create_order({"ingredients": ["60d3b41abdacab0026a733c6"]}, token)
    ...
    def test_create_order_with_incorrect_hash_error(self):
        token = None
        order = Order()
        response = order.create_order({"ingredients": ["60d3b"]}, token)
